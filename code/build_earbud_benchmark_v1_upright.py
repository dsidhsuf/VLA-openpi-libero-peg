...
def build_single_episode_benchmark(args, episode_dir: Path):
    ...
    level = episode_dir.parent.name
    if level not in LEVEL_TO_BDDL:
        raise ValueError(
            f"Could not infer level from parent folder '{level}'. "
            f"Expected one of {sorted(LEVEL_TO_BDDL)}."
        )
    ...
    if not is_upright(meta):
        print(f"[warn] selected episode is not upright according to metadata: {episode_dir}")

    ok = is_success(meta)
    if (not args.include_fail) and (not ok):
        raise RuntimeError(
            f"Selected episode is not marked successful: {episode_dir}. "
            "Use --include-fail if you still want to build this benchmark."
        )
    ...
    seed = int(meta.get("seed", parse_seed_from_name(episode_dir.name)))
    state_file = states_dir / f"{level}_init_states.npz"
    np.savez_compressed(
        state_file,
        qpos=np.asarray([qpos], dtype=np.float32),
        qvel=np.asarray([qvel], dtype=np.float32),
        episode_dir=np.asarray([str(episode_dir)], dtype=object),
        seed=np.asarray([seed], dtype=np.int32),
    )

    tasks = {
        "created_at": datetime.now().isoformat(),
        "dataset_root": str(episode_dir.parent.parent.resolve()),
        "single_episode": str(episode_dir.resolve()),
        "levels": {
            level: {
                "name": f"earbud_insert_{level}_upright_single",
                "bddl": bddl_path,
    ...
    print(f"[single] level: {level}")
    print(f"[single] episode: {episode_dir}")
    print(f"[saved] tasks: {tasks_json}")
    print(f"[saved] module: {args.module_path.resolve()}")
    print(f"[count] {level} = 1")
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Build a benchmark from exactly this raw episode directory.",
    )
    ...
    if args.single_episode is not None:
        build_single_episode_benchmark(args, args.single_episode.resolve())
        return

    if args.dataset_root is None:
        raise ValueError("--dataset-root is required unless --single-episode is provided.")
