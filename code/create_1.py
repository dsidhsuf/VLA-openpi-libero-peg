...
def rollout(
    ...
    if quality_mode not in ("balanced", "compact", "full"):
        raise ValueError(f"Unsupported quality_mode={quality_mode!r}; use balanced, compact, or full.")
    compact_mode = bool(compact_mode or quality_mode == "compact")
    balanced_mode = quality_mode == "balanced"

    def quality_steps(full_steps, balanced_steps=None, compact_steps=None, minimum=1):
...
def main():
    ...
    parser.add_argument(
        "--quality-mode",
        choices=["balanced", "compact", "full"],
        default=DEFAULT_QUALITY_MODE,
        help="balanced removes redundant holds/gripper repeats; compact is shorter; full keeps the old long flow.",
    )
    parser.add_argument(
        "--full-video",
        action="store_true",
