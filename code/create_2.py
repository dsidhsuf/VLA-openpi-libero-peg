...
def rollout(
    ...
    if quality_mode not in ("vla", "balanced", "compact", "full"):
        raise ValueError(f"Unsupported quality_mode={quality_mode!r}; use vla, balanced, compact, or full.")
    compact_mode = bool(compact_mode or quality_mode == "compact")

    def quality_steps(full_steps, vla_steps=None, balanced_steps=None, compact_steps=None, minimum=1):
    ...
    preclose_steps = quality_steps(PRECLOSE_STEPS, balanced_steps=12, compact_steps=8)
    close_gripper_steps = quality_steps(CLOSE_GRIPPER_STEPS, balanced_steps=14, compact_steps=10)
    rotate_settle_steps = quality_steps(ROTATE_SETTLE_STEPS, balanced_steps=2, compact_steps=1)
    post_move_hold_steps = quality_steps(POST_MOVE_HOLD_STEPS, balanced_steps=2, compact_steps=1)
    pre_release_hold_steps = quality_steps(PRE_RELEASE_HOLD_STEPS, balanced_steps=3, compact_steps=1)
    release_hold_steps = quality_steps(RELEASE_HOLD_STEPS, balanced_steps=4, compact_steps=2)
    safe_up_steps = quality_steps(100, vla_steps=45, balanced_steps=65, compact_steps=35)
    move_above_object_steps = quality_steps(160, vla_steps=65, balanced_steps=95, compact_steps=45)
    grasp_yaw_steps = quality_steps(MAX_ROTATE_STEPS, vla_steps=90, balanced_steps=130, compact_steps=70)
    pregrasp_steps = quality_steps(160, vla_steps=60, balanced_steps=90, compact_steps=45)
    preclose_height_steps = quality_steps(160, vla_steps=55, balanced_steps=85, compact_steps=40)
    descend_cage_steps = quality_steps(180, vla_steps=80, balanced_steps=110, compact_steps=60)
    lift_steps = quality_steps(180, vla_steps=70, balanced_steps=105, compact_steps=55)
    object_axis_align_steps = quality_steps(MAX_OBJECT_YAW_ALIGN_STEPS, vla_steps=140, balanced_steps=220, compact_steps=100)
    move_safe_slot_steps = quality_steps(260, vla_steps=95, balanced_steps=150, compact_steps=75)
    slot_hover_steps = quality_steps(320, vla_steps=140, balanced_steps=200, compact_steps=110)
    slot_fine_rotate_steps = quality_steps(MAX_OBJECT_YAW_ALIGN_STEPS, vla_steps=100, balanced_steps=180, compact_steps=80)
    descend_pre_insert_steps = quality_steps(320, vla_steps=150, balanced_steps=220, compact_steps=120)
    descend_final_insert_steps = quality_steps(360, vla_steps=180, balanced_steps=260, compact_steps=140)
    retreat_steps = quality_steps(160, vla_steps=55, balanced_steps=90, compact_steps=45)
    ...
    if quality_mode != "full":
        print(
            ...
            f"descend_cage={descend_cage_steps}, insert=({descend_pre_insert_steps},{descend_final_insert_steps})"
        )
...
def main():
    ...
    parser.add_argument(
        "--quality-mode",
        choices=["vla", "balanced", "compact", "full"],
        default=DEFAULT_QUALITY_MODE,
        help="vla is smooth and concise; balanced is safer; compact is shorter; full keeps the old long flow.",
    )
    parser.add_argument(
        "--full-video",
        action="store_true",
