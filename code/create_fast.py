...
class EpisodeRecorder:
    def __init__(
        ...
        if self.record_mode not in ("all", "keyframe"):
            raise ValueError(f"Unsupported record_mode={self.record_mode!r}; use all or keyframe.")
...
def rollout(
    ...
    if quality_mode not in ("train", "train_fast", "vla", "balanced", "compact", "full"):
        raise ValueError(
            f"Unsupported quality_mode={quality_mode!r}; use train, train_fast, vla, balanced, compact, or full."
        )
    compact_mode = bool(compact_mode or quality_mode == "compact")
    record_mode = str(record_mode or DEFAULT_RECORD_MODE).lower()
    if record_mode not in ("all", "keyframe"):
        raise ValueError(f"Unsupported record_mode={record_mode!r}; use all or keyframe.")

    def quality_steps(full_steps, train_steps=None, vla_steps=None, balanced_steps=None, compact_steps=None, minimum=1):
    ...
    preclose_steps = quality_steps(PRECLOSE_STEPS, train_steps=18, balanced_steps=12, compact_steps=8)
    close_gripper_steps = quality_steps(CLOSE_GRIPPER_STEPS, train_steps=24, balanced_steps=14, compact_steps=10)
    rotate_settle_steps = quality_steps(ROTATE_SETTLE_STEPS, train_steps=2, balanced_steps=2, compact_steps=1)
    post_move_hold_steps = quality_steps(POST_MOVE_HOLD_STEPS, train_steps=2, balanced_steps=2, compact_steps=1)
    ...
    lift_steps = quality_steps(180, train_steps=160, vla_steps=70, balanced_steps=105, compact_steps=55)
    object_axis_align_steps = quality_steps(MAX_OBJECT_YAW_ALIGN_STEPS, train_steps=320, vla_steps=140, balanced_steps=220, compact_steps=100)
    move_safe_slot_steps = quality_steps(260, train_steps=220, vla_steps=95, balanced_steps=150, compact_steps=75)
    slot_hover_steps = quality_steps(320, train_steps=260, vla_steps=140, balanced_steps=200, compact_steps=110)
    slot_fine_rotate_steps = quality_steps(MAX_OBJECT_YAW_ALIGN_STEPS, train_steps=260, vla_steps=100, balanced_steps=180, compact_steps=80)
    descend_pre_insert_steps = quality_steps(320, train_steps=260, vla_steps=150, balanced_steps=220, compact_steps=120)
    descend_final_insert_steps = quality_steps(360, train_steps=300, vla_steps=180, balanced_steps=260, compact_steps=140)
    retreat_steps = quality_steps(160, train_steps=110, vla_steps=55, balanced_steps=90, compact_steps=45)
...
def main():
    ...
    parser.add_argument(
        "--quality-mode",
        choices=["train", "train_fast", "vla", "balanced", "compact", "full"],
    ...
    parser.add_argument(
        "--record-mode",
        choices=["all", "keyframe"],
        default=DEFAULT_RECORD_MODE,
        help="all records every control step; keyframe records fewer synchronized image/state/action samples.",
    )
    parser.add_argument(
        "--require-success",
        action="store_true",
