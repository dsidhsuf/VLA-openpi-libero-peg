...
def rollout(
    ...
    if quality_mode not in ("train", "train_fast", "train_insert_fast", "vla", "balanced", "compact", "full"):
        raise ValueError(
            f"Unsupported quality_mode={quality_mode!r}; use train, train_fast, train_insert_fast, vla, balanced, compact, or full."
        )
    ...
    move_safe_slot_steps = quality_steps(
        260, train_steps=220, train_insert_fast_steps=120, vla_steps=95, balanced_steps=150, compact_steps=75
    )
    slot_hover_steps = quality_steps(
        320, train_steps=260, train_insert_fast_steps=100, vla_steps=140, balanced_steps=200, compact_steps=110
    )
    ...
    descend_pre_insert_steps = quality_steps(
        320, train_steps=260, train_insert_fast_steps=110, vla_steps=150, balanced_steps=220, compact_steps=120
    )
    descend_final_insert_steps = quality_steps(
        360, train_steps=300, train_insert_fast_steps=120, vla_steps=180, balanced_steps=260, compact_steps=140
    )
    retreat_steps = quality_steps(
        160, train_steps=110, train_insert_fast_steps=55, vla_steps=55, balanced_steps=90, compact_steps=45
    )
    ...
    if quality_mode != "full":
        print(
            ...
            f"descend_cage={descend_cage_steps}, move_slot={move_safe_slot_steps}, "
...
def main():
    ...
    parser.add_argument(
        "--quality-mode",
        choices=["train", "train_fast", "train_insert_fast", "vla", "balanced", "compact", "full"],
