#!/usr/bin/env python3
"""
Query a running PI0 policy server on real LeRobot training frames and compare
predicted actions against dataset labels.

This is useful when rollout fails even on a benchmark built from the same
episode: if the policy cannot predict label-like actions on the original
training frames, the problem is training/config/input-format rather than
closed-loop rollout.
"""
...
def infer(server: str, payload: dict) -> np.ndarray:
...
def select_indices(actions: np.ndarray, max_samples: int) -> list[int]:
    ...
    # Beginning of the episode, strong z-down frames, gripper transitions,
    # strong xy frames, and uniform coverage.
    add(range(min(20, n)))
    if n > 1:
        grip = actions[:, -1]
        add(np.argsort(np.abs(np.diff(grip, prepend=grip[0])))[-max_samples:])
    add(np.argsort(actions[:, 2])[:max_samples])  # most negative z
...
def main():
    ...
    parser.add_argument("--output-csv", default="training_frame_replay_compare.csv")
    ...
    parser.add_argument("--default-task", default="Insert the earbud into the charging slot.")
    ...
    for out_i, idx in enumerate(indices, start=1):
        sample = ds[idx]
        task = sample.get("task", args.default_task)
        ...
        payload = {
            ...
            args.image_key: encode_image_to_b64(sample[args.image_key]),
            args.image2_key: encode_image_to_b64(sample[args.image2_key]),
        }
        pred = infer(args.server, payload).astype(np.float32).reshape(-1)
