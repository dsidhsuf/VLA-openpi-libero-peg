...
def run_one_episode(env, init_state, instruction, max_steps, server_url: str, record=False, video_cameras=None):
    env.reset()
    base = env.env if hasattr(env, "env") else env
...
def main():
    ...
    parser.add_argument("--record_limit_per_level", type=int, default=3)  # 0=all
    ...
    for task in tasks:
        ...
        for i, init_state in enumerate(init_states):
            record_this = args.record_video and (args.record_limit_per_level <= 0 or i < args.record_limit_per_level)
