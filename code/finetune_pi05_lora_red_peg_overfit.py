#!/usr/bin/env python3
"""
Wide action-expert LoRA fine-tuning launcher for PI0.5/PI0 LIBERO checkpoints.

This script is intended for small-dataset overfit checks. By default it freezes
the vision encoder and main VLM side, but attaches LoRA adapters to a wider
action-expert path than the earlier q/v-only recipe:
  - gemma_expert self-attention q/k/v/o projections
  - gemma_expert MLP gate/up/down projections
...
# Wide PI0.5 overfit target. This avoids LoRA on the heavy vision_tower by
# default, but includes the language trunk, multimodal projector, action expert,
# and action/time projection heads. The previous action-expert-only target was
# too narrow for single-trajectory overfit on the red-peg task.
...
def patch_processor_module_for_local_tokenizer(module_name: str, label: str) -> Path | None:
    """Patch a PI processor module to allow a local PaliGemma tokenizer path."""
    try:
        processor_module = importlib.import_module(module_name)
    except Exception as exc:
        print(f"[compat] Could not import {label} processor for tokenizer patch: {exc}")
        return None

    processor_py = Path(inspect.getfile(processor_module)).resolve()
    ...
    if needle not in text:
        print(f"[compat] Could not patch local tokenizer for {label}; pattern not found in {processor_py}")
        return processor_py
    ...
    print(f"[compat] Patched {label} processor tokenizer path: {processor_py}")
    print(f"[compat] Backup saved at: {backup}")
    return processor_py


def patch_pi_processors_for_local_tokenizer() -> list[Path]:
    patched = []
    for module_name, label in (
        ("lerobot.policies.pi0.processor_pi0", "PI0"),
        ("lerobot.policies.pi05.processor_pi05", "PI0.5"),
    ):
        path = patch_processor_module_for_local_tokenizer(module_name, label)
        if path is not None:
            patched.append(path)
    return patched
...
def main() -> None:
    ...
    parser.add_argument(
        "--dataset-path",
        default="/root/autodl-tmp/openpi_earbud_proto/lerobot_ready/earbud_better_red_peg_single",
    ...
    parser.add_argument(
        ...
        help="Base PI0.5/PI0 checkpoint path.",
    )
    parser.add_argument(
        "--output-root",
        default="/root/autodl-tmp/openpi_earbud_proto/outputs_pi05_lora_overfit",
    )
    parser.add_argument("--dataset-repo-id", default="local/earbud_better_red_peg_single")
    parser.add_argument("--job-name", default="pi05_lora_red_peg_overfit")
    ...
    parser.add_argument(
        ...
        choices=["true", "false"],
        default="true",
        help="Use true for the first action-expert overfit check; set false to allow broader trainable scope.",
    )
    parser.add_argument(
        "--freeze-vision-encoder",
        choices=["true", "false"],
        default="true",
        help="Keep true for faster/safer overfit checks; set false only if action/language tuning still fails.",
    )
    ...
    parser.add_argument("--scheduler-decay-steps", type=int, default=4000)
    parser.add_argument("--scheduler-decay-lr", type=float, default=1e-6)
    parser.add_argument(
        "--tokenizer-path",
    ...
    if patched_tokenizer_paths:
        print("patched_processor_tokenizers:")
        for path in patched_tokenizer_paths:
            print(f"  {path}")
    print("")
    ...
    summary = {
        ...
        "patched_processor_tokenizers": [str(path) for path in patched_tokenizer_paths],
        "command": cmd,
        "args": vars(args),
    }
