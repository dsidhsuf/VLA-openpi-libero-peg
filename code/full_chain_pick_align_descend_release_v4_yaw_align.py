...
# ===== 只新增：随机 yaw + 局部 yaw 对齐 =====
DEFAULT_YAW_MIN = -20.0
DEFAULT_YAW_MAX = 20.0
...
# 孔的目标方向：先用 0 deg（即回到成功基线的默认方向）
# 如果你发现“长边/宽边”还是反了，把它改成 90.0
TARGET_SLOT_YAW_DEG = 0.0
...
def yaw_from_quat_wxyz(q):
    # obs["robot0_eef_quat"] 在你的日志里看起来是 wxyz
    w, x, y, z = q
...
def rollout(level: str, seed: int, yaw_min: float, yaw_max: float):
    ...
    out_dir = f"/root/autodl-tmp/openpi_earbud_proto/full_chain_release_v4_yaw_align/{level}"
    ...
    # ===== reset 后固定 earbud 和 slot：保留成功基线结构，只给 earbud 增加随机 yaw =====
    ...
    def debug_state(tag):
        ...
        eef_yaw = yaw_from_quat_wxyz(obs["robot0_eef_quat"])
    ...
    # ===== 新增：只在两个局部阶段用 z-yaw 对齐 =====
    def make_yaw_align_action(hold_pos, yaw_err_deg, gripper_cmd, yaw_sign, pos_clip=POS_CLIP_RELEASE):
        cur_pos = obs["robot0_eef_pos"]
        pos_err = hold_pos - cur_pos

        yaw_cmd = yaw_sign * np.clip(KP_YAW * np.deg2rad(yaw_err_deg), -YAW_CLIP, YAW_CLIP)
    ...
    def probe_yaw_sign(hold_pos, gripper_cmd):
        yaw0 = yaw_from_quat_wxyz(obs["robot0_eef_quat"])

        action = np.zeros(7, dtype=np.float32)
        pos_err = hold_pos - obs["robot0_eef_pos"]
        action[:3] = np.clip(KP_POS * pos_err, -POS_CLIP_RELEASE, POS_CLIP_RELEASE)
        action[5] = YAW_PROBE_CMD
        action[6] = gripper_cmd

        step_and_record(action)
        step_and_record(action)

        yaw1 = yaw_from_quat_wxyz(obs["robot0_eef_quat"])
        dyaw = wrap_deg(yaw1 - yaw0)

        # 尝试回一点，减少 probe 的残留影响
        action[5] = -YAW_PROBE_CMD
        step_and_record(action)
        step_and_record(action)
        ...
        if abs(dyaw) < 0.1:
            return 1.0
        return 1.0 if dyaw > 0 else -1.0

    def align_eef_yaw(hold_pos, target_yaw_deg, gripper_cmd, yaw_sign, steps=YAW_ALIGN_STEPS):
        for _ in range(steps):
            cur_yaw = yaw_from_quat_wxyz(obs["robot0_eef_quat"])
            err = wrap_deg(target_yaw_deg - cur_yaw)
            if abs(err) < YAW_TOL_DEG:
                break
            action = make_yaw_align_action(hold_pos, err, gripper_cmd, yaw_sign)
            step_and_record(action)
    ...
    base_eef_yaw_deg = yaw_from_quat_wxyz(obs["robot0_eef_quat"])

    print("seed:", seed)
    print("rand_yaw_deg:", round(rand_yaw_deg, 3))
    print("base_eef_yaw_deg:", round(base_eef_yaw_deg, 3))
    ...
    # 探测 yaw action 的正方向
    yaw_sign = probe_yaw_sign(safe_up_pos, GRIP_OPEN)
    print("yaw_action_sign:", yaw_sign)
    ...
    # ===== 新增：抓取前 yaw 对齐 =====
    target_grasp_yaw_deg = base_eef_yaw_deg + rand_yaw_deg
    print("[phase 1.5] yaw align above object")
    align_eef_yaw(safe_above_earbud, target_grasp_yaw_deg, GRIP_OPEN, yaw_sign)
    debug_state("after_yaw_align_grasp")
    ...
    # ===== 新增：抓起后把 yaw 回正到孔方向 =====
    target_slot_yaw_deg = base_eef_yaw_deg + TARGET_SLOT_YAW_DEG
    print("[phase 7.5] yaw align after lift")
    align_eef_yaw(lift_pos, target_slot_yaw_deg, GRIP_CLOSE, yaw_sign)
    debug_state("after_yaw_align_slot")
    ...
    video_path = os.path.join(out_dir, f"full_chain_pick_align_descend_release_v4_yaw_align_seed{seed}_{ts}.mp4")
