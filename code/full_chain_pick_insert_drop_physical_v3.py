...
# ===== 关键修复：用你之前真实抓取成功时的 earbud 初始 pose =====
EARBUD_FIXED_INIT_POS = np.array([0.07318314, -0.10582792, 0.45821508], dtype=float)
EARBUD_FIXED_INIT_QUAT_WXYZ = np.array([0.70710678, 0.0, -0.70710678, 0.0], dtype=float)
...
def rollout(level: str):
    ...
    out_dir = f"/root/autodl-tmp/openpi_earbud_proto/full_chain_pick_insert_drop_physical_v3/{level}"
    ...
    def enforce_earbud_init():
        ...
        q_ear[:3] = EARBUD_FIXED_INIT_POS
        q_ear[3:7] = EARBUD_FIXED_INIT_QUAT_WXYZ
    ...
    safe_above_earbud = np.array([
        EARBUD_FIXED_INIT_POS[0] + GRASP_X_OFFSET,
        EARBUD_FIXED_INIT_POS[1] + GRASP_Y_OFFSET,
        SAFE_TRAVEL_Z
    ], dtype=float)
    ...
    pregrasp_pos = np.array([
        EARBUD_FIXED_INIT_POS[0] + GRASP_X_OFFSET,
        EARBUD_FIXED_INIT_POS[1] + GRASP_Y_OFFSET,
        EARBUD_FIXED_INIT_POS[2] + PREGRASP_Z_OFFSET
    ], dtype=float)

    preclose_pos = np.array([
        EARBUD_FIXED_INIT_POS[0] + GRASP_X_OFFSET,
        EARBUD_FIXED_INIT_POS[1] + GRASP_Y_OFFSET,
        EARBUD_FIXED_INIT_POS[2] + PRECLOSE_Z_OFFSET
    ], dtype=float)

    cage_pos = np.array([
        EARBUD_FIXED_INIT_POS[0] + GRASP_X_OFFSET,
        EARBUD_FIXED_INIT_POS[1] + GRASP_Y_OFFSET,
        EARBUD_FIXED_INIT_POS[2] + CAGE_Z_OFFSET
    ], dtype=float)
    ...
    z_lift = earbud_pos_lift[2] - EARBUD_FIXED_INIT_POS[2]
    eef_obj_dist_lift = np.linalg.norm(eef_pos_lift - earbud_pos_lift)
    physical_grasp_success = (z_lift > 0.03) and (eef_obj_dist_lift < 0.08)

    print(f"\nphysical_grasp_success={physical_grasp_success}")
    print(f"z_lift={z_lift:.4f}")
    print(f"eef_obj_dist_lift={eef_obj_dist_lift:.4f}")

    if not physical_grasp_success:
        png_path = os.path.join(out_dir, f"failed_grasp_{ts}.png")
        mp4_path = os.path.join(out_dir, f"failed_grasp_{ts}.mp4")
    ...
    png_path = os.path.join(out_dir, f"full_chain_pick_insert_drop_physical_v3_{ts}.png")
    mp4_path = os.path.join(out_dir, f"full_chain_pick_insert_drop_physical_v3_{ts}.mp4")
