...
# 用真实抓取成功后，在 partial insert 阶段估计的相对位姿
# 这里不是从头悬空夹取，而是“抓取成功之后”再切换成 carry/drop 模式
DROP_OBJ_MINUS_EEF = np.array([0.0077, -0.0001, -0.0492], dtype=float)
...
def rollout(level: str):
    ...
    out_dir = f"/root/autodl-tmp/openpi_earbud_proto/full_chain_pick_insert_drop/{level}"
    os.makedirs(out_dir, exist_ok=True)

    frames = []

    # ===== 固定 slot；earbud 初始阶段用 baseline 的稳定初始化 =====
    ...
    # 在完成真实抓取并移动到 slot 上方后，切换为 carry follow 模式
    ...
    def enforce_earbud_follow_eef():
        ...
        earbud_pos = eef_pos + DROP_OBJ_MINUS_EEF
        q_ear = get_joint_qpos(sim, earbud_joint_name)
        q_ear[:3] = earbud_pos
    ...
    # ===== 第二部分：切换到成功的 drop 插入策略 =====
    # 从这里开始，不再依赖物理接触保持抓取，而是切到“携带载荷”的插入子任务
    follow_payload = True

    desired_obj_hover = SLOT_FIXED_POS.copy()
    desired_obj_hover[2] = SLOT_FIXED_POS[2] + SLOT_HOVER_Z_OFFSET
    desired_eef_hover = desired_obj_hover - DROP_OBJ_MINUS_EEF
    ...
    print("[phase 11] hold before release")
    ...
    png_path = os.path.join(out_dir, f"full_chain_pick_insert_drop_{ts}.png")
    mp4_path = os.path.join(out_dir, f"full_chain_pick_insert_drop_{ts}.mp4")
