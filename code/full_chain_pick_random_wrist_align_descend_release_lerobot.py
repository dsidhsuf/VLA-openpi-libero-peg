...
DEFAULT_CAMERA_NAMES = ("agentview", "robot0_eye_in_hand", "frontview")
...
def rollout(
    ...
    effective_video_fps = int(max(1, round(capture_hz * playback_speed))) if video_fps <= 0 else int(max(1, video_fps))
    print(
        f"video_config: capture_hz={capture_hz:.2f}, "
        f"playback_speed={playback_speed:.2f}, "
        f"video_fps={effective_video_fps}, "
...
def main():
    ...
    parser.add_argument(
        ...
        help="Output video fps. <=0 means auto = (control_hz / frame_stride) * playback_speed.",
    )
    ...
    parser.add_argument(
        ...
        help="Only affects video playback speed when --video-fps <= 0.",
    )
    parser.add_argument(
        "--full-video",
        action="store_true",
        help="If set, keep all rehearsal/hold segments and export longer videos.",
    )
    ...
    for ep_idx in range(int(max(1, args.episodes))):
        ...
        metadata = rollout(
            ...
            compact_mode=(not args.full_video),
        )
