...
def rollout(level: str, seed: int, random_yaw_min_deg: float, random_yaw_max_deg: float, flat_rest_prob: float, record_root: str = ""):
    cfg = LEVEL_CFG[level]
    ...
    def step_and_record(action, phase_name="unknown"):
        nonlocal obs
        enforce_slot()
        ...
        if "agentview_image" in obs:
            ...
            img_upright = img[::-1].copy()
            frames.append(img_upright)
            record["images_agentview"].append(img_upright)

        if "robot0_proprio-state" in obs:
            record["state"].append(np.asarray(obs["robot0_proprio-state"], dtype=np.float32).copy())
        else:
            record["state"].append(np.array([], dtype=np.float32))

        record["action"].append(np.asarray(action, dtype=np.float32).copy())
        record["phase"].append(phase_name)
    ...
    def servo_to_pos(target_pos, grip_cmd, steps=MAX_SERVO_STEPS, pos_tol=0.004, phase_name="move"):
    ...
    def servo_to_pos_slow(target_pos, grip_cmd, steps=220, pos_tol=0.003, phase_name="move_slow"):
        for _ in range(steps):
    ...
    def servo_yaw_hold_pos(target_pos, target_eef_yaw_deg, grip_cmd, steps=MAX_ROTATE_STEPS, phase_name="yaw_align"):
        for _ in range(steps):
            eef_yaw = current_eef_yaw_deg()
    ...
    def servo_rpy_hold_pos(target_pos, target_rpy_deg, grip_cmd, steps=MAX_ROTATE_STEPS, clip_val=POS_CLIP_RELEASE, phase_name="rpy_align"):
    ...
    def servo_object_yaw_hold_pos(target_pos, target_object_yaw_deg, grip_cmd, steps=MAX_OBJECT_YAW_ALIGN_STEPS, phase_name="object_yaw_align"):
        for _ in range(steps):
    ...
    def servo_object_pose_to_target(
        ...
        phase_name="object_pose_to_target",
    ):
        for _ in range(steps):
    ...
    def command_gripper_to_target(target_pos, command, target_abs, mode="open", max_steps=80, phase_name="gripper"):
    ...
    servo_to_pos(safe_up_pos, GRIP_OPEN, steps=100, pos_tol=0.006, phase_name="safe_up")
    debug_state("after_safe_up")

    print("[phase 0.5] air close")
    for _ in range(18):
        step_and_record(make_pose_action(obs["robot0_eef_pos"], GRIP_CLOSE), "air_close")
    debug_state("after_air_close")

    print("[phase 0.6] air open")
    for _ in range(18):
        step_and_record(make_pose_action(obs["robot0_eef_pos"], GRIP_OPEN), "air_open")
    debug_state("after_air_open")

    print("[phase 1] move above object")
    servo_to_pos(safe_above_earbud, GRIP_OPEN, steps=160, pos_tol=0.005, phase_name="safe_xy")
    ...
    if rest_pose_mode == "flat":
        ...
        servo_rpy_hold_pos(safe_above_earbud, flat_target_rpy_deg, GRIP_OPEN, steps=MAX_ROTATE_STEPS, phase_name="grasp_pose_align")
        ...
        servo_rpy_hold_pos(side_pregrasp, flat_target_rpy_deg, GRIP_OPEN, steps=200, phase_name="flat_side_pregrasp")
        ...
        servo_rpy_hold_pos(side_contact, flat_target_rpy_deg, GRIP_OPEN, steps=200, phase_name="flat_side_contact")
        debug_state("after_flat_side_contact")

        print("[phase 3] preclose gripper at flat contact")
        for _ in range(PRECLOSE_STEPS):
            step_and_record(make_pose_action(side_contact, GRIP_CLOSE, rot_cmd=np.array([0.0, 0.0, 0.0]), clip_val=POS_CLIP_RELEASE), "preclose")
        ...
        servo_rpy_hold_pos(flat_center, flat_target_rpy_deg, GRIP_CLOSE, steps=220, phase_name="flat_center_grasp")
        debug_state("after_descend")

        print("[phase 5] squeeze")
        for _ in range(CLOSE_GRIPPER_STEPS):
            step_and_record(make_pose_action(flat_center, GRIP_CLOSE, rot_cmd=np.array([0.0, 0.0, 0.0]), clip_val=POS_CLIP_RELEASE), "close")
        debug_state("after_close")

        print("[phase 6] lift")
        lift_pos = obs["robot0_eef_pos"].copy() + np.array([0.0, 0.0, lift_z_offset])
        servo_rpy_hold_pos(lift_pos, flat_target_rpy_deg, GRIP_CLOSE, steps=220, phase_name="lift")
        ...
        print(f"object_axis_for_grasp_deg={object_yaw_for_grasp_deg:.2f}")
        print(f"target_eef_yaw_for_grasp_deg={target_eef_yaw_for_grasp_deg:.2f}")
        servo_yaw_hold_pos(safe_above_earbud, target_eef_yaw_for_grasp_deg, GRIP_OPEN, steps=MAX_ROTATE_STEPS, phase_name="grasp_yaw_align")
        ...
        servo_to_pos(pregrasp_pos, GRIP_OPEN, steps=160, pos_tol=0.004, phase_name="pregrasp")
        debug_state("after_pregrasp")

        print("[phase 3] preclose height")
        servo_to_pos(preclose_pos, GRIP_OPEN, steps=160, pos_tol=0.003, phase_name="preclose_pos")
        debug_state("after_preclose_pos")

        print("[phase 4] preclose gripper")
        for _ in range(PRECLOSE_STEPS):
            step_and_record(make_pose_action(preclose_pos, GRIP_CLOSE), "preclose")
        debug_state("after_preclose")

        print("[phase 5] descend to cage")
        servo_to_pos(cage_pos, GRIP_CLOSE, steps=180, pos_tol=0.002, phase_name="descend")
        debug_state("after_descend")

        print("[phase 6] squeeze")
        for _ in range(CLOSE_GRIPPER_STEPS):
            step_and_record(make_pose_action(cage_pos, GRIP_CLOSE), "close")
        debug_state("after_close")

        print("[phase 7] lift")
        lift_pos = obs["robot0_eef_pos"].copy() + np.array([0.0, 0.0, lift_z_offset])
        servo_to_pos(lift_pos, GRIP_CLOSE, steps=180, pos_tol=0.003, phase_name="lift")
        debug_state("after_lift")
    ...
    if rest_pose_mode == "flat":
        ...
        servo_rpy_hold_pos(restore_pos, restore_rpy_deg, GRIP_CLOSE, steps=MAX_ROTATE_STEPS, phase_name="flat_restore_upright")
        debug_state("after_flat_restore_upright")

        print("[phase 6.6] settle after flat restore")
        for _ in range(ROTATE_SETTLE_STEPS):
            step_and_record(make_pose_action(restore_pos, GRIP_CLOSE, rot_cmd=np.array([0.0, 0.0, 0.0]), clip_val=POS_CLIP_RELEASE), "flat_restore_settle")
    ...
    servo_object_yaw_hold_pos(rotate_anchor_pos, target_earbud_axis_deg, GRIP_CLOSE, steps=MAX_OBJECT_YAW_ALIGN_STEPS, phase_name="rotate_align")
    debug_state("after_rotate_align")

    print("[phase 7.6] settle after wrist rotation")
    for _ in range(ROTATE_SETTLE_STEPS):
        step_and_record(make_pose_action(rotate_anchor_pos, GRIP_CLOSE, rot_cmd_z=0.0, clip_val=POS_CLIP_RELEASE), "rotate_settle")
    ...
    servo_to_pos(safe_above_slot, GRIP_CLOSE, steps=260, pos_tol=0.005, phase_name="safe_slot")
    ...
    servo_object_pose_to_target(
        ...
        yaw_tol_deg=5.0,
        clip_val=POS_CLIP_RELEASE,
        phase_name="slot_hover",
    )
    debug_state("after_slot_hover")

    print("[phase 9.5] fine rotate above slot")
    slot_rotate_anchor_pos = obs["robot0_eef_pos"].copy()
    servo_object_yaw_hold_pos(slot_rotate_anchor_pos, target_earbud_axis_deg, GRIP_CLOSE, steps=MAX_OBJECT_YAW_ALIGN_STEPS, phase_name="slot_fine_rotate")
    debug_state("after_slot_fine_rotate")

    print("[phase 9.6] settle after fine rotate")
    for _ in range(ROTATE_SETTLE_STEPS):
        step_and_record(make_pose_action(slot_rotate_anchor_pos, GRIP_CLOSE, rot_cmd_z=0.0, clip_val=POS_CLIP_RELEASE), "slot_fine_rotate_settle")
    debug_state("after_slot_fine_rotate_settle")

    print("[phase 10] hold above slot")
    for _ in range(POST_MOVE_HOLD_STEPS):
        step_and_record(make_pose_action(obs["robot0_eef_pos"].copy(), GRIP_CLOSE), "slot_hold")
    ...
    servo_object_pose_to_target(
        ...
        yaw_tol_deg=7.0,
        clip_val=POS_CLIP_RELEASE,
        phase_name="final_insert_descend",
    )
    ...
    for _ in range(PRE_RELEASE_HOLD_STEPS):
        step_and_record(make_pose_action(desired_eef_release, GRIP_CLOSE, clip_val=POS_CLIP_RELEASE), "pre_release_hold")
    ...
    for _ in range(RELEASE_HOLD_STEPS):
        step_and_record(make_pose_action(desired_eef_release, GRIP_OPEN, clip_val=POS_CLIP_RELEASE), "drop_hold")
    debug_state("after_drop_hold")

    print("[phase 15] retreat upward")
    retreat_pos = obs["robot0_eef_pos"].copy()
    retreat_pos[2] += RETREAT_Z
    servo_to_pos(retreat_pos, GRIP_OPEN, steps=160, pos_tol=0.004, phase_name="retreat")
    ...
    if record_root and release_drop_success:
        episode_dir = os.path.join(record_root, f"ep_{level}_seed{seed:04d}_{ts}")
        os.makedirs(episode_dir, exist_ok=True)

        images = np.stack(record["images_agentview"], axis=0).astype(np.uint8)
        states = np.stack(record["state"], axis=0).astype(np.float32)
        actions = np.stack(record["action"], axis=0).astype(np.float32)
        phases = np.asarray(record["phase"], dtype=object)

        np.savez_compressed(
            os.path.join(episode_dir, "episode.npz"),
            images_agentview=images,
            state=states,
            action=actions,
            phase=phases,
        )
        ...
        meta = {
            ...
            "slot_body_yaw_deg": float(slot_body_yaw_deg),
            "slot_long_axis_deg": float(slot_long_axis_deg),
        ...
        imageio.mimwrite(os.path.join(episode_dir, "preview.mp4"), images, fps=20)
        print("saved_episode_dir:", episode_dir)
...
def main():
    ...
    parser.add_argument("--record-root", type=str, default="")
