...
# ===== yaw 对齐参数 =====
# 默认让细柱长边对齐孔的长边（+90 deg yaw）
# 如果你发现方向还是反了，把它改成 -90.0
EARBUD_YAW_DEG = 90.0
...
def rollout(level: str):
    ...
    out_dir = f"/root/autodl-tmp/openpi_earbud_proto/full_chain_release_v2/{level}"
    ...
    # 再绕 z 轴加 yaw 对齐
    q_yaw = quat_wxyz_from_axis_angle([0, 0, 1], EARBUD_YAW_DEG)
    earbud_stable_quat = quat_mul_wxyz(q_yaw, q_vertical)
    ...
    video_path = os.path.join(out_dir, f"full_chain_pick_align_descend_release_v2_{ts}.mp4")
