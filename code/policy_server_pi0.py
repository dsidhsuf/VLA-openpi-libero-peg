...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Base full PI0 checkpoint path used when --policy_path points to a PEFT/LoRA adapter. "
            "For LoRA checkpoints, set this to /root/autodl-tmp/hf_models/pi0_libero_base."
        ),
    )
    ...
    if is_adapter:
        if not args.base_policy_path:
            raise RuntimeError(
                ...
                "Example: --base_policy_path /root/autodl-tmp/hf_models/pi0_libero_base"
            )
    ...
    print(f"[server] loading pi0_libero_base from: {load_policy_path}")
