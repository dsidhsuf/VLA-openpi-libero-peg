...
# 不再“张着夹爪”直接压到物体中心
PRECLOSE_Z_OFFSET = 0.028   # 在物体上方先闭合
CAGE_Z_OFFSET = 0.014       # 闭合后再轻压包住
LIFT_Z_OFFSET = 0.10
...
def rollout(level):
    ...
    out_dir = f"/root/autodl-tmp/openpi_earbud_proto/robot_grasp_sanity_v2/{level}"
    ...
    eef_quat0 = obs["robot0_eef_quat"].copy()
    ...
    print("[phase 0.5] air grip test close")
    for _ in range(18):
        action = make_pos_only_action(obs["robot0_eef_pos"], GRIP_CLOSE)
        step_and_record(action)
    debug_state("after_air_close")

    print("[phase 0.6] air grip test open")
    for _ in range(18):
        action = make_pos_only_action(obs["robot0_eef_pos"], GRIP_OPEN)
        step_and_record(action)
    debug_state("after_air_open")

    print("[phase 1] move above earbud at safe height")
    servo_to_pos(safe_above_earbud, GRIP_OPEN, steps=160, pos_tol=0.005)
    debug_state("after_safe_xy")

    print("[phase 2] lower to pregrasp")
    servo_to_pos(pregrasp_pos, GRIP_OPEN, steps=160, pos_tol=0.004)
    debug_state("after_pregrasp")

    # 关键修复：先在物体上方“收夹爪”，不要张着嘴直接怼
    print("[phase 3] lower to preclose height")
    servo_to_pos(preclose_pos, GRIP_OPEN, steps=160, pos_tol=0.003)
    debug_state("after_preclose_pos")

    print("[phase 4] preclose gripper above object")
    for _ in range(PRECLOSE_STEPS):
        if episode_done:
            break
        action = make_pos_only_action(preclose_pos, GRIP_CLOSE)
        step_and_record(action)
    ...
    for _ in range(CLOSE_GRIPPER_STEPS):
        if episode_done:
            break
        action = make_pos_only_action(cage_pos, GRIP_CLOSE)
        step_and_record(action)
    ...
    for _ in range(HOLD_STEPS):
        if episode_done:
            break
        action = make_pos_only_action(lift_pos, GRIP_CLOSE)
        step_and_record(action)
    debug_state("after_hold")
    ...
    video_path = os.path.join(out_dir, f"robot_grasp_sanity_v2_{ts}.mp4")
