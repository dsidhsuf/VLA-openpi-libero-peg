...
def rollout(level: str):
    ...
    env = OffScreenRenderEnv(
        ...
        ignore_done=True,   # 关键修复：不要在抓起后提前终止
    )
    env.seed(0)
    ...
    out_dir = f"/root/autodl-tmp/openpi_earbud_proto/robot_pick_to_slot_demo_success/{level}"
    ...
    slot_stable_quat = np.array([1.0, 0.0, 0.0, 0.0], dtype=float)  # 先强制竖直
    ...
    carry_success = grasp_success and (obj_slot_xy < 0.04) and (earbud_pos_final[2] > slot_pos0[2] + 0.04)
    ...
    video_path = os.path.join(out_dir, f"robot_pick_to_slot_demo_success_{ts}.mp4")
