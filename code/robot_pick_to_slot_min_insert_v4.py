...
TARGET_PRECLOSE_ABS = 0.0112
TARGET_SQUEEZE_ABS = 0.0108
...
def get_joint_qvel(sim, joint_name):
    try:
        return np.array(sim.data.get_joint_qvel(joint_name), dtype=float)
    except Exception:
        return None
...
def set_joint_qvel_zero(sim, joint_name):
    qvel = get_joint_qvel(sim, joint_name)
    if qvel is None:
        return
    try:
...
def rollout(level: str):
    ...
    out_dir = f"/root/autodl-tmp/openpi_earbud_proto/robot_pick_to_slot_min_insert_v4/{level}"
    ...
    def command_gripper_to_target(target_pos, command, target_abs, mode="close", max_steps=80):
    ...
    print("[phase 0.5] air close to target")
    command_gripper_to_target(obs["robot0_eef_pos"], GRIP_CLOSE, TARGET_PRECLOSE_ABS, mode="close", max_steps=60)
    debug_state("after_air_close")

    print("[phase 0.6] air open to target")
    command_gripper_to_target(obs["robot0_eef_pos"], GRIP_OPEN, TARGET_OPEN_ABS, mode="open", max_steps=60)
    ...
    print("[phase 4] preclose gripper to target")
    command_gripper_to_target(preclose_pos, GRIP_CLOSE, TARGET_PRECLOSE_ABS, mode="close", max_steps=80)
    ...
    print("[phase 6] squeeze to target")
    command_gripper_to_target(cage_pos, GRIP_CLOSE, TARGET_SQUEEZE_ABS, mode="close", max_steps=80)
    ...
    servo_to_pos(desired_eef_insert, GRIP_CLOSE, steps=180, pos_tol=0.0035)
    ...
    video_path = os.path.join(out_dir, f"robot_pick_to_slot_min_insert_v4_{ts}.mp4")
