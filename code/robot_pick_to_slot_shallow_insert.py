...
def rollout(level: str):
    ...
    out_dir = f"/root/autodl-tmp/openpi_earbud_proto/robot_pick_to_slot_shallow_insert/{level}"
    ...
    print("[phase 10] shallow descend")
    desired_eef_shallow = desired_eef_hover.copy()
    desired_eef_shallow[2] -= SHALLOW_INSERT_DELTA_Z
    servo_to_pos(desired_eef_shallow, GRIP_CLOSE, steps=220, pos_tol=0.0035)
    debug_state("after_shallow_insert")

    print("[phase 11] hold shallow insert")
    for _ in range(POST_INSERT_HOLD_STEPS):
        step_and_record(make_pos_only_action(desired_eef_shallow, GRIP_CLOSE))
    ...
    shallow_insert_success = carry_success and (obj_slot_xy < 0.02) and (obj_slot_z < SLOT_HOVER_Z_OFFSET)

    print(f"\nfinal grasp_success={grasp_success}")
    print(f"final carry_success={carry_success}")
    print(f"final shallow_insert_success={shallow_insert_success}")
    ...
    video_path = os.path.join(out_dir, f"robot_pick_to_slot_shallow_insert_{ts}.mp4")
