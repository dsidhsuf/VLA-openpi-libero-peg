...
def main():
    ...
    out_dir = f"/root/autodl-tmp/openpi_earbud_proto/levels_eval_rot/{args.level}"
    ...
    def move_earbud_to(target_pos, n_steps):
        ...
        for i in range(n_steps):
            ...
            log_obs("move step", obs2)
            record_current_frame(obs2)
            last_obs = obs2
        return last_obs

    def rotate_earbud_to(target_quat, n_steps):
        qpos = get_joint_qpos(sim, joint_name)
    ...
    obs = rotate_earbud_to(slot_quat, n_steps=14)
    ...
    mp4_path = os.path.join(out_dir, f"scripted_rollout_rot_{ts}.mp4")
