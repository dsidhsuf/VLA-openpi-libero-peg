...
OUT_DIR = "/root/autodl-tmp/openpi_earbud_proto/custom_task_scripted_pose"
...
def custom_success(obs, xy_thresh=0.03, z_thresh=0.025, angle_thresh_deg=25.0):
    ep = obs["earbud_1_pos"]
    ...
    # 先假设耳机长轴 = 局部 z 轴，槽轴 = 局部 z 轴
    # 如果后面发现角度明显不合理，再把 earbud_axis 改成 "x" 或 "y"
...
frames.extend([img] * 4)
imageio.imwrite(os.path.join(OUT_DIR, "init_scripted_pose.png"), img)
...
def move_earbud_to(target_pos, n_steps):
    ...
    for i in range(n_steps):
        ...
        s, xy, zz, ang = custom_success(obs2)
